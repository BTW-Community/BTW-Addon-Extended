...
public class DawnUtilsReflection {
	private static MinecraftServer mcServer = MinecraftServer.getServer();
	...
	public static void addCommand(ICommand command) {
		Field commandManagerField;
		
		try {
			if (isObfuscated()) {
				commandManagerField = MinecraftServer.class.getDeclaredField("p");
			}
			else {
				commandManagerField = MinecraftServer.class.getDeclaredField("commandManager");
			}

			commandManagerField.setAccessible(true);
			CommandHandler handler = (CommandHandler) commandManagerField.get(mcServer);
			handler.registerCommand(command);
			commandManagerField.setAccessible(false);
		} catch (NoSuchFieldException e) {
			if (isObfuscated()) {
				e.printStackTrace();
			}
